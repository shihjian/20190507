      ...
      <tbody>
        <tr v-for="(item) in products" :key="item.id" class="text-center">
          ...
          <td>
            ...
            <button class="btn btn-outline-danger mt-1 btn-sm" @click="deleteProduct(item.id)">刪除</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    updataProduct(){
      ...
      console.log('新增檔案',response.data);
    ...
    uploadFile(){
      ...
      if(response.data.success){
        ...
        vm.$set(vm.tempProduct,'imageUrl',response.data.imageUrl); //強行寫入
      }else{
